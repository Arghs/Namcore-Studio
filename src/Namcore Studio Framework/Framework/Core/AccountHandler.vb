                        ...
                        Case Is > 2
                            Dim accountDt As DataTable =
                                    ReturnDataTableRealm(
                                        "SELECT `" & GlobalVariables.sourceStructure.acc_name_col(0) & "`, `" &
                            ...
                            Dim tmpAccount As Account = account2Extend
                            If tmpAccount Is Nothing Then _
                                tmpAccount = New Account() With {.Name = _tempResult, .Id = accountId}
                            tmpAccount.SourceExpansion = GlobalVariables.sourceExpansion
                            tmpAccount.Core = GlobalVariables.sourceCore
                            LogAppend(
                                "Loaded account name info for accountId: " & accountId.ToString & " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", True)
                            _tempResult = accountDt.Rows(0).Item(1).ToString()
                            tmpAccount.PassHash = _tempResult
                            LogAppend(
                                "Loaded account passHash info for accountId: " & accountId.ToString & " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            ...
                            tmpAccount.Locale = TryInt(_tempResult)
                            LogAppend(
                                "Loaded account locale info for accountId: " & accountId.ToString & " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            _tempResult = accountDt.Rows(0).Item(4).ToString()
                            tmpAccount.JoinDate = DateTime.Parse(_tempResult)
                            LogAppend(
                                "Loaded account joindate info for accountId: " & accountId.ToString & " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            _tempResult = accountDt.Rows(0).Item(5).ToString()
                            tmpAccount.LastIp = _tempResult
                            LogAppend(
                                "Loaded account lastip info for accountId: " & accountId.ToString & " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            ...
                            LogAppend(
                                "Loaded account expansion information for accountId: " & accountId.ToString &
                                " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            'todo Expansion!
                            ...
                            Dim accountAccessDt As DataTable =
                                    ReturnDataTableRealm(
                                        "SELECT `" & GlobalVariables.sourceStructure.accAcc_gmLevel_col(0) & "`, `" &
                            ...
                            LogAppend(
                                "Loaded accountAccess gmlevel info for accountId: " & accountId.ToString &
                                " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            ...
                            LogAppend(
                                "Loaded accountAccess realmId info for accountId: " & accountId.ToString &
                                " and setId: " &
                                tarSetId &
                                " // result is: " & _tempResult, "AccountHandler_LoadAccount", False)
                            tmpAccount.SetIndex = tarSetId
