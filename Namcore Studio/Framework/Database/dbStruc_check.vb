...
Public Class dbStruc_check
    ...
    Dim tmp_connInfo As MySqlConnection
    Dim characterDB As String
    Dim authDB As String
    Dim exceptionCounter As Integer
    Dim DBReport As String
    ...
    Public Sub startCheck(ByVal coreString As String, ByVal expansion As Integer, ByVal sqlconn As MySqlConnection, ByVal realmsqlconn As MySqlConnection, ByVal infosqlconn As MySqlConnection, ByVal characterDBNAME As String, ByVal authDBNAME As String, ByVal target As Boolean)
        ...
        Select Case coreString
            Case "trinity"
                ...
                If xpac >= 3 Then check_character_glyphs(dbstruc)
    ...
    Private Sub check_characters(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_achievement(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_achievement_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_action(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_action_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_glyphs(ByVal struc As DBStructure)
        ...
        End If
        If Not DBReport = "" Then gettablescheme(struc.character_glyphs_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_homebind(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_homebind_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_inventory(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_inventory_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_queststatus(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_queststatus_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_queststatus_rewarded(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_queststatus_rewarded_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_reputation(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_reputation_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_skills(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_skills_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_spell(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_spells_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_character_talent(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.character_talent_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub check_item_instance(ByVal struc As DBStructure)
        ...
        If Not DBReport = "" Then gettablescheme(struc.item_instance_tbl(0), characterDB)
        DBReport = tmpReport & vbNewLine & DBReport
    End Sub
    Private Sub gettablescheme(ByVal table As String, ByVal db As String)
        tablescheme = tablescheme & "######## " & table & " ########" & vbNewLine
        Dim da As New MySqlDataAdapter("SELECT COLUMN_NAME FROM COLUMNS WHERE TABLE_NAME='" & table & "' AND TABLE_SCHEMA='" & db & "'", tmp_connInfo)
        Dim dt As New DataTable
        Try
            tmp_connInfo.Open()
        Catch ex As Exception
            tmp_connInfo.Close()
            tmp_connInfo.Dispose()
        End Try
        Try
            da.Fill(dt)
            Try
            ...
            If Not lastcount = 0 Then
                Do
                    ...
                    tablescheme = tablescheme & column & vbNewLine
                    count += 1
    ...
    Private Sub tbl_check(ByVal tablename() As String)
        ...
        Do
            ...
            Catch
                Select Case i
                    Case 1, counter + 1
                        LogAppend("Table " & tablename(counter) & " not found! -> No alternatives!", "dbStruc_check_tbl_check")
        ...
        Do
            ...
            Try
                ...
                If CInt(myData.HasRows) = 0 Then
                    ...
                    LogAppend("Using column " & columnname(counter) & " in table " & tablename(0) & "!", "dbStruc_check_col_check")
                    Exit Do
                Else
                    columnname(0) = columnname(counter)
                    LogAppend("Using column " & columnname(counter) & " in table " & tablename(0) & "!", "dbStruc_check_col_check")
            ...
            Catch
                Select Case i
                    Case 1, counter + 1
                        LogAppend("Column " & columnname(counter) & " not found in " & tablename(0) & "! -> No alternatives!", "dbStruc_check_col_check")
    ...
    Private Sub tbl_check_realm(ByVal tablename() As String)
        ...
        Do
            ...
            Dim sqlquery = ("SELECT * FROM " & tablename(counter) & " LIMIT 1")
            Dim myCommand As New MySqlCommand()
            myCommand.Connection = tmp_connRealm
            myCommand.CommandText = sqlquery
            myAdapter.SelectCommand = myCommand
            Dim myData As MySqlDataReader
            Try
